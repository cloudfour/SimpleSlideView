(function() {
  ...
  SimpleSlideView = (function() {
    ...
    SimpleSlideView.prototype._pushOrPop = function(action, pushResult, popResult) {
      if (pushResult == null) {
        pushResult = true;
      }
      if (popResult == null) {
    ...
    SimpleSlideView.prototype.changeView = function(targetView, action) {
      ...
      if (this.options.useTransformProps) {
        ...
        bothCSS['left'] = 0;
        targetCSS[transformProp] = translateBefore + this._pushOrPop(action, 100, -100) + '%' + translateAfter;
        outAnimProps[transformProp] = translateBefore + this._pushOrPop(action, -100, 100) + '%' + translateAfter;
        inAnimProps[transformProp] = translateBefore + '0' + translateAfter;
      } else {
        activeCSS['left'] = 0;
        targetCSS['left'] = this._pushOrPop(action, containerWidth, containerWidth * -1);
        outAnimProps['left'] = this._pushOrPop(action, containerWidth * -1, containerWidth);
        inAnimProps['left'] = 0;
      }
