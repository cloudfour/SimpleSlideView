(function() {
  var $, SimpleSlideView, defaults, outerHeight, outerWidth, resetStyles, scrollCallback;
  ...
  } : ($.scrollTo != null) && (typeof Zepto !== "undefined" && Zepto !== null) ? $.scrollTo : function(top, duration, callback) {
  ...
  SimpleSlideView = (function() {
    ...
    SimpleSlideView.prototype.changeView = function(targetView, action) {
      ...
      if (this.options.useTransformProps) {
        ...
        bothCSS['left'] = 0;
        targetCSS[transformProp] = translateBefore + this.pushOrPop(action, 100, -100) + '%' + translateAfter;
        ...
        activeCSS['left'] = 0;
        targetCSS['left'] = this.pushOrPop(action, containerWidth, containerWidth * -1);
      ...
      onChangeEnd = function() {
        ...
        if (_this.options.scrollCallback && _this.options.scrollOn === 'end') {
          return _this.scrollAction(onScrollEnd);
        } else {
          return onScrollEnd();
        }
      };
      ...
      if (this.options.scrollCallback && this.options.scrollOn === 'start') {
        return this.scrollAction(changeAction);
      } else {
        return changeAction();
      }
    };

    SimpleSlideView.prototype.scrollAction = function(callback) {
      var top;
      top = this.options.scrollToContainerTop ? this.$container.position().top : 0;
      if ($(window).scrollTop() > top) {
        return this.options.scrollCallback(top, this.options.duration, callback);
      } else {
        return callback();
      }
    };
