...
(function() {
  var $, $window, SimpleSlideView, backfaceVisibility, defaults, event, eventPrefix, isZepto, outerDimension, outerHeight, outerWidth, prefix, resetStyles, testEl, transform, transition, transitionEnd, vendor, vendors,
    __slice = [].slice;
  ...
  defaults = {
    ...
    scrollToPlugin: $.fn.scrollTo != null
  };
  ...
  outerDimension = function(el, dim) {
    var outerDim;

    outerDim = dim[0].toUpperCase() + dim.slice(1);
    return $(el)[$.fn[outerDim] != null ? outerDim : dim]();
  };

  outerHeight = function(el) {
    return outerDimension(el, 'height');
  };

  outerWidth = function(el) {
    return outerDimension(el, 'width');
  };
  ...
  SimpleSlideView = (function() {
    ...
    SimpleSlideView.prototype.animateJS = function($targetView, push, containerWidth) {
      ...
      this.$container.animate({
        ...
        return resetStyles(_this.$container, ['height', 'overflow', 'position', 'width']);
      });
      this.scrollToTop();
    ...
    SimpleSlideView.prototype.animateCSS = function($targetView, push, containerWidth) {
      ...
      $window.on(transitionEnd, function(event) {
        if (event.target === $targetView[0]) {
          ...
          _this.$container.css('height', outerHeight($targetView));
    ...
    SimpleSlideView.prototype.scrollToTop = function() {
      ...
      if (this.options.scrollToPlugin != null) {
